...
public class C07_EnUzunEnKisakelimeyazdir {
    ...
    public static void enUzunEnKisaYazdir(String[] isimler){
        ...
        for (int i = 0; i < isimler.length ; i++) {
            if (isimler[i].length() > enUzunKelime.length()){ // esittir olmadıgından ilk en uzun degeri alıyor
                enUzunKelime = isimler[i];
            }
            if (isimler[i].length() < enKisaKelime.length()){ //  esittir olmadıgından ilk en kısa degeri alıyor
